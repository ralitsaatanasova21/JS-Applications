import { deleteBookById, getBookById, getLikeBookFromUser, likeBook, totalLikesForBook } from "../api/data.js";
...
<section id="details-page" class="details">
    <div class="book-information">
        ...
        <div class="actions">
        ${buttonsTemplate(book, isOwner, onDelete)}

        ${likesTemplate(showLikeButton, onLike)}
...
const buttonsTemplate=(book, isOwner, onDelete)=>{
    if(isOwner){
        return html`
...
const likesTemplate=(showLikeButton, onLike)=>{
...
export async function detailsPage(ctx){
    ...
    const [book, likes, hasLike]= await Promise.all([
        ...
        totalLikesForBook(ctx.params.id),
        userData ? getLikeBookFromUser(ctx.params.id, userData.id) : 0
    ])
